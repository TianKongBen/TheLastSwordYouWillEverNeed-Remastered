        ...
        if(!world.isClientSide() && world.getGameTime() % 10 == 0) {
            if(entity instanceof Player player) {
                ...
                if(isWearingDragonCrystalArmor(player)) {
                    if(player.getEffect(MobEffects.NIGHT_VISION) == null || Objects.requireNonNull(player.getEffect(MobEffects.NIGHT_VISION)).getDuration() < 250){
                        player.addEffect(new MobEffectInstance(MobEffects.NIGHT_VISION, 10, 127, false, true, false));
                    }

                    if(player.getEffect(MobEffects.DIG_SPEED) == null || Objects.requireNonNull(player.getEffect(MobEffects.DIG_SPEED)).getDuration() < 250){
                        player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED, 10, 127, false, true, false));
                    }

                    if(player.getEffect(MobEffects.DAMAGE_BOOST) == null || Objects.requireNonNull(player.getEffect(MobEffects.DAMAGE_BOOST)).getDuration() < 250){
                        player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, 10, 127, false, true, false));
                    }

                    if(player.getEffect(MobEffects.DAMAGE_RESISTANCE) == null || Objects.requireNonNull(player.getEffect(MobEffects.DAMAGE_RESISTANCE)).getDuration() < 250){
                        player.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 10, 127, false, true, false));
                    }
                }
            }
        }
