...
public class RenderLD extends EntityRenderer<LDEntity> {
    private static final Identifier TEXTURE = Identifier.ofVanilla("textures/item/diamond.png");
    ...
    private void renderModel(MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        MatrixStack.Entry entry = matrixStack.peek();
        VertexConsumer vertexConsumer = vertexConsumerProvider.getBuffer(LAYER);

        addVertex(vertexConsumer, entry, i, 0.0F, 0, 0, 1);
        addVertex(vertexConsumer, entry, i, 1.0F, 0, 1, 1);
        addVertex(vertexConsumer, entry, i, 1.0F, 1, 1, 0);
        addVertex(vertexConsumer, entry, i, 0.0F, 1, 0, 0);
    }

    private static void addVertex(VertexConsumer vertexConsumer, MatrixStack.Entry matrix, int light, float x, int y, int textureU, int textureV) {
        vertexConsumer.vertex(matrix, x - 0.5f, (float)y - 0.25f, 0.0f)
